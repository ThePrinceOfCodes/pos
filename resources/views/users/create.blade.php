                        ...
                        <form method="post" action="{{ route('users.store') }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('branch') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-method">Branch</label>
                                    <select name="branch" id="input-method"
                                        class="form-select form-control-alternative{{ $errors->has('branch') ? ' is-invalid' : '' }}" required>
                                        @foreach ($branches as $branch)
                                        @if($branch['name'] == old('branch'))
                                    ...
                                    @include('alerts.feedback', ['field' => 'branch'])
                                </div>

                                <div class="form-group{{ $errors->has('role') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-method">Role</label>
                                    <select name="role" id="input-method"
                                        class="form-select2 form-control-alternative{{ $errors->has('role') ? ' is-invalid' : '' }}" required>
