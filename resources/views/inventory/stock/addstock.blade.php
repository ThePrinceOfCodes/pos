@extends('layouts.app', ['page' => 'Add Product', 'pageSlug' => 'stock', 'section' => 'inventory'])
                    ...
                    <div class="card-header">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('stock.show', $stock) }}" class="btn btn-sm btn-primary">Back to List</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('stock.product.store', $stock) }}" autocomplete="off">
                            @csrf

                            <div class="pl-lg-4">
                                <input type="hidden" name="receipt_id" value="{{ $stock->id }}">
                                    ...
                                    <select name="product_id" id="input-product" class="form-select form-control-alternative{{ $errors->has('product_id') ? ' is-invalid' : '' }}" required>
                                        @foreach ($products as $product)
                                            @if($product['id'] == old('product_id'))
                                                <option value="{{$product['id']}}" selected>{{ "$product->name : [$product->store availiable]" }}</option>
                                            @else
                                                <option value="{{$product['id']}}">{{ "$product->name : [$product->store availiable]" }}</option>
                                            @endif
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group{{ $errors->has('stock') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="number" name="stock" id="input-stock" class="form-control form-control-alternative{{ $errors->has('stock') ? ' is-invalid' : '' }}" value="{{ old('stock') }}" required>
